...
export function placeOrderMessage(order: {order_total: number, order_items: any[], menu_items: any[]}):MessageDTO {
    const {order_total, order_items, menu_items} = order;
    let options = []
    menu_items.map((item) => {
        let order_item = order_items.filter(itm => itm.item_id === item.id)
    ...
    return {
        text: "Your order:",
        options,
        additional_text: `Total cost: ${order_total}. <br>Please type 'confirm' to confirm your order`
    }
